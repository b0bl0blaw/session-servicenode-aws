	...
	private createService(
		...
		const serviceNodeContainer = sessionTaskDefinition.addContainer(
			'session-service-node',
			{
				...
				image: ecs.ContainerImage.fromRegistry('b0bl0blawslawbl0g/session-sn-aws'),
				essential: true,
				privileged: false,
		...
		const storageServerContainer = sessionTaskDefinition.addContainer(
			'session-storage-server',
			{
				...
				image: ecs.ContainerImage.fromRegistry('b0bl0blawslawbl0g/session-ss-aws'),
				essential: false,
				privileged: false,
				logging: ecs.LogDriver.awsLogs(
					{
		...
		const lokinetContainer = sessionTaskDefinition.addContainer(
			'session-lokinet-server',
			{
				...
				image: ecs.ContainerImage.fromRegistry('b0bl0blawslawbl0g/session-lokinet-aws'),
				essential: false,
				privileged: false,
				logging: ecs.LogDriver.awsLogs(
					{
		...
		return new ecs.FargateService(
			this,
			'sessionFargateService',
			{
				cluster: cluster,
